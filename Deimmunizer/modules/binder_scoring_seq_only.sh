...
OUTPUT_DIR="$(realpath outDir)" #must be full path
SCRIPT_DIR="/dtu/projects/RFdiffusion/closed-loop/de_novo_binder_scoring"
AF3_WEIGHTS="/dtu/projects/RFdiffusion/closed-loop/de_novo_binder_scoring/af3_weights"
INPUT_CSV="$(realpath "$metrics_csv")"
...
# Count number of input PDB files. I commented this
...
#I changed this to use --mode seq_only_csv
...
--flash_attention_implementation=xla  #Rasmus. Crashed on gpuaV when commented
conda deactivate
conda deactivate
...
#Removed "af2": "'${OUTPUT_DIR}/AF2/pdbs'", from dict
echo '{"input": "'${OUTPUT_DIR}/input_pdbs'", "colab": "'${OUTPUT_DIR}/ColabFold/pdbs'", "boltz1": "'${OUTPUT_DIR}/Boltz/pdbs'", "af3": "'${OUTPUT_DIR}/AF3/pdbs'"}' > "${PYMOL_DIR}/pdb_dirs.json"
...
TIME_PER_STRUC=$(echo "scale=2; $RUN_TIME / 1" | bc) #Changed this to run with csv
